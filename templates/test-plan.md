...
> **Date**: [date]
> **Generated by**: the qa-plan Skill
...
**What to test**:
- [Formula or rule from GDD Formulas section — e.g., "damage = base * multiplier where multiplier in [0.5, 3.0]"]
...
Critical paths to verify before QA hand-off (run via the smoke-check Skill):
...
- [ ] Smoke check passes (run the smoke-check Skill before QA hand-off)
- [ ] No regressions introduced — previous sprint's features still pass
- [ ] Code reviewed (via the code-review Skill or documented peer review)
- [ ] Story file updated to `Status: Complete` via the story-done Skill
...
*Template: `docs-src/templates/test-plan.md`*
*Generated by: the qa-plan Skill — do not edit this line*
