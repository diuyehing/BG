...
2. **Ask architecture questions:**
   ...
   - *Ask the user in an Issue comment — batch constrained architecture questions*
...
6. **Complete the story with the story-done Skill:**
   - When implementation (and tests, if written) is complete, invoke the story-done Skill with the story file path
   - This verifies acceptance criteria, checks for GDD/ADR deviations, prompts code review, and updates the story status
   - Do NOT mark a story complete manually — always go through the story-done Skill
   - If no story file exists for this work (ad-hoc task), offer the code-review Skill directly instead
...
File is ready. Would you like me to:
 ...
 C) Run the code-review Skill for architecture validation"
...
You: [creates tests/combat/test_damage_calculator.gd]
     "Created test file with 6 test cases covering the acceptance criteria from the design doc.
      ...
      Running the story-done Skill to verify acceptance criteria and close out the story."

[the story-done Skill runs — verifies criteria, checks deviations, prompts code review, updates story status]
```
...
- Story completion is explicit — use the story-done Skill to close every story, never assume done because code is written

#### Structured Decision UI

Use Issue comments for architecture decisions and next-step choices.
Follow the **Explain → Capture** pattern:

1. **Explain first** — Describe the architectural options and trade-offs in
   the Issue comment.
2. **Capture the decision** — Ask the user with concise option labels.

**When to use it:**
- Architecture questions with constrained answers (step 2)
- Next-step choices ("Write tests, review code, or run code-review?")
- Batch up to 4 independent architecture questions in one Issue comment

**When NOT to use it:**
- Open-ended spec clarifications — use conversation
- Single confirmations ("May I write to file?")
- When working as an agent in an Issue thread — structure text for the user to review directly

**Example — architecture questions (batch):**

  Issue comment with questions:
