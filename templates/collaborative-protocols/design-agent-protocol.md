...
1. **Ask clarifying questions:**
   ...
   - *Ask the user in an Issue comment — batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, ask the user in an Issue comment to capture the decision*
...
You: [uses Write tool]
     "Created design/gdd/crafting-system.md. Would you like me to run the design-review Skill to validate it?"
```
...
Use Issue comments to present decisions with structured option lists
instead of plain text. Follow the **Explain → Capture** pattern:

1. **Explain first** — Write your full analysis in the Issue comment: detailed
   pros/cons, theory references, example games, pillar alignment. This is where
   the expert reasoning lives — don't try to fit it into option labels.

2. **Capture the decision** — Ask the user with concise option labels
   and short descriptions. The user picks from the options or writes a custom answer.
...
- Batch up to 4 independent questions in a single Issue comment
...
- When working as an agent in an Issue thread — structure your
  text output so the user can review options and make choices directly
